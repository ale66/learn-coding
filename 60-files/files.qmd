...
- of variable lenght  
- any elem. can be accessed via the `[]` notation:

`mylist[i]` or `mydict['mykey']`
...
- data structures exist only at run-time: they disappear as soon as the execution ends
  
- data is made permanent via the *file system*

- files have a *format* which 
  
  plays a similar role as types for variables
  
  guides interpretation, memorization and exchange

Yet, Python can open *any* file, even AV ones

File: a mere succession of characters, terminated by the special char. EOF 
...
- the hierarchical file system structure: folders 
- unique names in each folder
- changeable content but permanent memorisation

There could be two or more copies ot `pg100.txt` 

but only one `C:/Users/aless/git/learn-code/60-files/pg100.txt` 
...
## The `with` instruction
  ...
  - copies the whole content from the permanet memory to the (volatile) work memory, and 
...
```python
  ...
  with open(MYWORKFOLDER+MYFILE) as myhandler:

      mytext = myhandler.read()
```

By default the file content is read as text, `.txt` is not considered
...
here `mytext` is a *blob* or mega-string. 
...
print(mytext)
```
The file handler `f` is accessible only from the block under `with`


## The working directory

VS Code normally starts in the top user folder/directory:
...
Manually set the absolute path to the data files:
...
receive a string and parse its characters, then 

create Latin-style all-caps text:
...
`\n` is used as separator to create a list of strings
...
Handling large files is better done by having `f` as an iterator

Example: `pg100.txt`, the complete Shakespare works

iterate over lines coming out of the handler
 (the default setup)
...
#### Challenge: can you get fresh data from [data.spectator.co.uk](https://data.spectator.co.uk/) and display it?


[![](./imgs/the_spectator.png)](https://data.spectator.co.uk/category/energy)
