...
- Python run-time data structures are *impermanent:* they disappear as soon as the execution ends
...
In principle Python can open *any* file, even AV ones

files are seen as a mere succession of characters, terminated by the special character EOF 
...
## The `with` instruction
  ...
  with open(MYFILE) as myhandler:
    mytext = myhandler.read()
  ```

  - the known file format determines what to do next
